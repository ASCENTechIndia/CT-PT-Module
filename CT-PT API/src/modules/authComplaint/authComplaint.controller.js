...
async function getCompListForSup(req, res, next) {
  ...
  VAR_CTPTSTAGE_NAME: row.VAR_CTPTSTAGE_NAME
    ...
    logApiSuccess( req, 200, { count: cleanData?.length || 0 }, 'Complaint List for Supervisor completed' ); 
...
async function getCompListForSI(req, res, next) {
  ...
  VAR_EMPCTPTENTRY_SIREMARK: row.VAR_EMPCTPTENTRY_SIREMARK
    ...
    logApiSuccess( req, 200, { count: cleanData?.length || 0 }, 'Complaint List for SI completed' );
    return res.ok({ data: cleanData, pagination });
  } catch (error) {
...
async function resolvedListbyVendor(req, res, next) {
   try { 
    const { ulbid, supervisorId, fromDate, toDate, status, page = 1, limit = 10 } = req.query;
...
async function getSolvedComplaintImagesCon(req, res, next) {
  try {
    ...
    const rows = await getSolvedComplaintImagesService(ulbid, siid, complaintid);
    logApiSuccess(req, 200, { count: rows?.length || 0 }, 'Solved complaint images retrieved successfully');
    return res.ok(rows);
  } catch (error) {
    logApiError(req, 500, error.message, 'Error retrieving solved complaint images');
    return next(error);
  }
}
...
async function getSupervisorStatusCon(req, res, next) {
  try {
    ...
    logApiSuccess(req, 200, { count: statuses?.length || 0 }, 'Supervisor status options retrieved successfully');
    return res.ok(statuses);
  } catch (error) {
    logApiError(req, 500, error.message, 'Error retrieving supervisor status options');
    return next(error);
  }
}
