...
async function compListforSupRepo(ulbid, cursor = 0, limit = 10) {
  let sql = ` SELECT * FROM ( SELECT e.num_empctptentry_id, 
  e.dat_empctptentry_date, e.var_empctptentry_latitude,
   e.num_empctptentry_id AS uniqueid, 
   u.var_user_username AS username, e.var_empctptentry_userid AS userid,
    e.var_empctptentry_longitude, e.num_empctptentry_toiletid, e.num_empctptentry_stageid,
     e.var_empctptentry_remark, e.dat_empctptentry_insdate, e.num_empctptentry_ulbid,
      e.bolb_empctptentry_image, e.bolb_empctptentry_image2, e.bolb_empctptentry_image3,
       ctpt.num_ctpttype_wardid, ctpt.var_ctpttype_toiletlocation, ctpt.var_ctpttype_femaleseats,
        ctpt.var_ctpttype_maleseats, ctpt.var_ctpttype_totalseats, ctpt.var_ctpttype_status,
         ctpt.var_ctpttype_username, st.var_ctptstage_status, st.var_ctptstage_name, 
         ROW_NUMBER() OVER ( PARTITION BY e.var_empctptentry_userid, e.num_empctptentry_toiletid, 
         ...
         AND num_empctptentry_id > :cursor ORDER BY num_empctptentry_id desc FETCH NEXT :limit ROWS ONLY `;
  const binds = {
    ulbid: Number(ulbid),
  ...
  const nextCursor =
    rows.length > 0 ? rows[rows.length - 1].NUM_EMPCTPTENTRY_ID : null;
  return { data: rows, nextCursor, limit: Number(limit) };
}

async function compListforSIRepo(ulbid, cursor = 0, limit = 10) {
  let sql = `
SELECT *
 FROM (
    SELECT
      ...
      e.bolb_empctptentry_image,
      e.bolb_empctptentry_image2,
      e.bolb_empctptentry_image3,
